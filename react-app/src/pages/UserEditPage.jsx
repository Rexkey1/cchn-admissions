...
export default function UserEditPage() {
  ...
  useEffect(() => {
    getUser(id).then(r => setForm({ ...r.data.row, password:'' })).catch(() => navigate('/users'))
  }, [id])

  async function submit(e) {
    e.preventDefault(); setErrors([])
    setLoading(true)
    try { await updateUser(id, form); navigate('/users') }
    catch(err) { setErrors([err.response?.data?.error || 'Failed to update.']) }
  ...
  return (
    <div className="animate-fadeIn" style={{ maxWidth:560, margin:'0 auto' }}>
      <div style={{ marginBottom:'1.5rem', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
        <div>
          <h1 style={{ fontSize:24, fontWeight:900, letterSpacing:'-0.5px' }} className="gradient-text">Edit User</h1>
          <p style={{ fontSize:13, color:'#64748b', marginTop:2 }}>Update user: <strong>{form.username}</strong></p>
        </div>
        ...
        <form onSubmit={submit} style={{ display:'grid', gap:'1.25rem' }}>
          {fields.map(f => (
            <div key={f.name}>
              ...
              <input type={f.type} value={form[f.name]||''} onChange={e=>setForm(p=>({...p,[f.name]:e.target.value}))} placeholder={f.placeholder||f.label} required={f.required} style={{ width:'100%', padding:'0.6rem 0.75rem', border:'1.5px solid var(--color-border)', borderRadius:10, fontSize:13, background:'#f8fafc', outline:'none', fontFamily:'inherit' }} />
            </div>
          ))}
          <div>
