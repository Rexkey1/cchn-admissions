...
import { getVerified, exportUrl } from '../api/client'
...
export default function VerifiedPage() {
  ...
  const fetch = useCallback(async (pg = page) => {
    setLoading(true)
    try {
      const r = await getVerified({ q, program: prog, page: pg })
  ...
  return (
    <div className="animate-fadeIn">
      <div style={{ marginBottom:'1.5rem', display:'flex', alignItems:'flex-start', justifyContent:'space-between', flexWrap:'wrap', gap:'1rem' }}>
        <div>
          <h1 style={{ fontSize:24, fontWeight:900, letterSpacing:'-0.5px' }} className="gradient-text">Verified Applicants</h1>
          <p style={{ fontSize:13, color:'#64748b', marginTop:2 }}>Applicants who have been verified by admins.</p>
        </div>
        <a href={exportUrl('verified', prog)} download style={{
      ...
      <div className="card" style={{ padding:'0.75rem', display:'flex', gap:'0.75rem', marginBottom:'1.25rem', flexWrap:'wrap' }}>
        <div style={{ position:'relative', flex:2, minWidth:160 }}>
          ...
          <input value={q} onChange={e=>setQ(e.target.value)} placeholder="Search…" style={{ width:'100%', paddingLeft:36, paddingRight:12, paddingTop:9, paddingBottom:9, border:'1.5px solid var(--color-border)', borderRadius:10, fontSize:13, background:'#f8fafc', outline:'none', fontFamily:'inherit' }} />
        </div>
        ...
        <span style={{ fontSize:13, fontWeight:600, color:'#64748b', alignSelf:'center' }}>{pagination.total.toLocaleString()} total</span>
      </div>
          ...
          <table style={{ width:'100%', borderCollapse:'collapse', fontSize:13 }}>
            <thead>
              <tr style={{ background:'#f8fafc' }}>
                {['#','Applicant','Program','Contact','Source','Paid'].map(h => (
            ...
            <tbody>
              {!loading && rows.length === 0 && <tr><td colSpan={6} style={{ textAlign:'center', padding:'3rem', color:'#94a3b8', fontWeight:700 }}>No verified applicants found</td></tr>}
              {rows.map((r, i) => (
                <tr key={r.id} style={{ borderTop:'1px solid #f1f5f9' }}>
                  ...
                  <td style={{ padding:'0.875rem 1rem' }}>
                    {r.is_paid == 1 && <span style={{ fontSize:10, fontWeight:700, padding:'2px 8px', borderRadius:6, background:'#dbeafe', color:'#1e40af' }}>Paid</span>}
                  </td>
                </tr>
              ))}
            </tbody>
