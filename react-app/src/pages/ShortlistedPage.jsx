...
export default function ShortlistedPage() {
  ...
  const fetch = useCallback(async (pg = 1) => {
    setLoading(true);
    try {
      const res = await api.getShortlisted({ q, program: prog, page: pg });
  ...
  const handleExport = () => {
    const url = api.getExportUrl('shortlisted', prog);
  ...
  return (
    <div className="animate-fadeIn">
      <div style={{ marginBottom: '2rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '1rem' }}>
        <div>
          <h1 style={{ fontSize: '26px', fontWeight: 900, letterSpacing: '-0.5px' }} className="gradient-text">Shortlisted Applicants</h1>
          <p style={{ color: 'var(--color-text-muted)', fontSize: '13px', fontWeight: 500, marginTop: '2px' }}>Total {pagination.total.toLocaleString()} shortlisted.</p>
        </div>
      ...
      <div className="card" style={{ padding: '0.75rem', marginBottom: '1.5rem', display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
        <div style={{ flex: 2, minWidth: '200px', position: 'relative' }}>
          ...
          <input value={q} onChange={e => setQ(e.target.value)} placeholder="Search shortlist..." style={{ width: '100%', padding: '0.625rem 1rem 0.625rem 2.5rem', borderRadius: '10px', border: '1.5px solid var(--color-border)', outline: 'none', background: '#f8fafc', fontSize: '13px', fontWeight: 500 }} />
        </div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
            <thead>
              <tr style={{ background: '#f8fafc' }}>
                ...
                <th style={{ padding: '1rem', textAlign: 'left', fontWeight: 800, color: '#64748b', textTransform: 'uppercase' }}>Phone</th>
              ...
              {rows.map((r, i) => (
                <tr key={r.id} style={{ borderTop: '1px solid #f1f5f9' }}>
                  ...
                  <td style={{ padding: '0.875rem 1rem' }}>
                    ...
                    <div style={{ fontSize: '11px', color: '#94a3b8', fontFamily: 'monospace' }}>{r.pin_moh || ''}</div>
                  </td>
                  <td style={{ padding: '0.875rem 1rem', fontWeight: 700, color: r.program === 'Diploma' ? '#4f46e5' : '#7c3aed' }}>{r.program}</td>
                  <td style={{ padding: '0.875rem 1rem', color: '#444' }}>{r.phone_number}</td>
                  <td style={{ padding: '0.875rem 1rem', color: '#475569' }}>{r.interview_date || <span style={{ color: '#cbd5e1' }}>Not set</span>}</td>
                </tr>
              ))}
            </tbody>
          </table>
